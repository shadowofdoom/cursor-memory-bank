 ...
 */
function parseArgs(): { port?: number, transport?: string } {
  const args = process.argv.slice(2);
  const options: { port?: number, transport?: string } = {};
...
async function main() {
  try {
    ...
    const server = new MemoryBankMCPServer(workspacePath, options.port);
    await server.start();
