...
export class MemoryBankManager {
  ...
  private isClinemoryBank: boolean = false;
   ...
   */
  public async detectClinemoryBank(): Promise<boolean> {
    try {
      ...
      const hasClinemoryBankStructure = files.some(file => 
        file.includes('projectbrief.md') || 
...
`;
   ...
   */
  public async initializeGlobalRules(): Promise<GlobalRules> {
   ...
   * Read global rules
   */
  public async readGlobalRules(): Promise<GlobalRules | null> {
    try {
      const rulesDir = this.globalRulesPath;
   ...
   * Update global rules
   */
  public async updateGlobalRules(content: string): Promise<GlobalRules> {
    const rulesDir = this.globalRulesPath;
    await fs.ensureDir(rulesDir);
   ...
   */
  private generateDefaultGlobalRule(): string {
